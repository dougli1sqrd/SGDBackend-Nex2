...
class EditMetadata extends Component {
  ...
  renderFileDrop() {
    ...
    return  (<Dropzone name={'file'} onDrop={this.handleDrop.bind(this)} multiple={false}>
             ...
             </Dropzone><div>Note: No need to upload a file if you only want to update the metadata for this file. If you upload a file and the md5sum is different from the current version, the interface will insert the metadata from this screen into the database, upload this new version to s3, set this version as 'Active', and mark the old version as 'Archived'.</div>);
  }
