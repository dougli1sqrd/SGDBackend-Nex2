...
import { setDisease } from '../../actions/funccompActions';
import { DataList } from 'react-datalist-field';
import Loader from '../../components/loader';
import PropTypes from 'prop-types';
const GET_ECO = '/eco';
const GET_DO = '/do';
const DISEASES = '/funccomp';
    ...
    if (this.state.isUpdate) {
      var buttons = this.state.list_of_diseases.filter((i, index) => {
        ...
        .map((disease, index) => {
          ...
          return <li key={new_index} onClick={() => this.handleSelectFunccomp(new_index)} className={`button medium-only-expanded ${this.state.currentIndex == new_index ? 'success' : ''}`}>{disease.dbentity_id.display_name}</li>;
        }
        );
      return (
        <div>
